...
app = FastAPI(title="mindctl Dashboard API")
...
async def get_stats():
    ...
    if not file_path.exists():
        return {"stats": "Aucune analyse disponible"}
    return {"stats": file_path.read_text(encoding='utf-8')}
...
async def get_depguard():
    ...
    conflits = []
    # Parsing par bloc (séparés par des lignes vides ou un délimiteur logique)
